...
**The MFT scanner has never been checked against the walk.** `Mft_scanner_agrees_with_the_walk_when_elevated` needs `$MFT`, so unelevated it returns at its first line — which is every run, now by declaration rather than by accident. It has been executed exactly once, by the elevated run that exposed all of the above, and **it failed**. That is an open finding about either the scanner or the test, not a passing test: what §22.4's comparison promises for this pair is not yet delivered. Recorded here rather than left to be rediscovered.
