...
| Условие | Поведение |
|---|---|
| Двойной клик из Проводника (консоль создана под нас) | интерактивная сессия, окно держится открытым |
| `--interactive` / `-i` | интерактивная сессия принудительно, из любого терминала |
...
*Реализовано в P3:* `--scanner mft|walk` (без значения — auto), `--no-elevate`, `--parallelism`, `--note`, `--top`, `--no-save`, `--quiet`; добавлен `--pause` — его подставляет сам инструмент при перезапуске с правами, чтобы новое окно консоли не закрылось до того, как пользователь увидит итог. Остальное — по мере фаз (`--full` в P9, `--exclude` в P7, `--format`/`--output` в P4).
...
| Walk-скан C:, 1.2 млн файлов | < 150 с | **измерено 44 с** с hardlink-дедупликацией для файлов ≥ 1 МБ (16 тыс. handle'ов ≈ 3 с); 41 с без неё |
...
| RSS при walk-скане 1.2 млн | < 400 МБ | **измерено 380 МБ** пик working set (P3, с hardlink-дедупликацией; 535 МБ до выноса file id из `RawEntry` в боковой список). Живой снимок при этом 94 МБ (62 байта/узел) — остальное транзиентный GC-хип фазы обхода. |
