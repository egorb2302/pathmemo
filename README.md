...
| **Allocated size** | Bytes actually occupied: cluster rounding, NTFS compression, sparse holes. | MFT run lists / logical size rounded up to the cluster, `GetCompressedFileSize` for compressed and sparse files only |
...
| 1M files | **3–8 s** | ~12 s on an SSD, minutes on an HDD |
| Allocated size | yes, free | cluster rounding, free; `GetCompressedFileSize` (+1 syscall) for compressed and sparse files |
...
**Allocated size is cluster rounding, not a syscall.** `FILE_FULL_DIR_INFO` carries the logical size and the attributes. For a file that is neither `Compressed` nor `SparseFile` the allocation is that size rounded up to the volume's cluster, and only for those two is `GetCompressedFileSize` asked — they are the only files it has a different answer for; for any other it returns the logical size. The walk used to ask it for every file, which cost 1.2M syscalls a scan and produced the wrong number anyway (§22.5): dropping the call took an unelevated walk of C: from 41–61 s to 13.5 s. The audit's `DirectoryMeasure` measured the same way from the start (§6.4).
...
│   │   │                           pure), ReclaimView (rules, then one rule's
│   │   │                           paths; reports, never deletes), ScanView
│   │   └── Work/                   ScanJob (a scan on a worker; posts, never
│   │                               touches the window), ReclaimJob (the rules
│   │                               over a snapshot, the same way)
│   │
...
**Every push is built and tested by `.github/workflows/ci.yml`**, separately from releasing. The reason is the same failure: the compile error below sat on `master` and was found by the tag push — the one moment when red means there is nothing to download. CI runs the tests and a `win-x64` publish, because trimming, ReadyToRun and the single-file host only run at publish time and an `IL2xxx` warning is a build failure here (§18). The release job keeps the arm64 cross-build, the zips, the hashes and the notes: five minutes that say nothing extra about whether the code is sound.
...
- [x] With nothing scanned yet, **Tree** and a volume card say there is nothing to browse and the window stays open - the case v0.2.0 got wrong, where either one killed the process outright (§24.6). Verified on a real store with no snapshots in it, by clicking, not by reasoning.
...
**The suite declares whether it is elevated; it does not inherit it.** `TestStore.Init` fixes the answer to *no* for the whole assembly, and `TestStore.Use` verifies the store actually moved to the temporary directory before a test writes anything. This is not tidiness. `--data-dir` is deliberately ignored while elevated (§12.1), so run as an administrator the same setup code silently leaves the store pointing at the real one: the suite then wrote its fixtures into a developer's own snapshots and destroyed one of them, because `Imports_a_snapshot_that_has_no_row` saves a fixture as scan 7 and scan 7 already existed. A GitHub Windows runner is an administrator by default, which is how one push found it. The check in `Use` is the half that matters — it fails on the test's first line instead of quietly operating on real data, and it holds even if the declaration is later removed. §12.1's refusal has its own test now, which it never had, because until this seam existed the rule could only be observed by running the suite in the one configuration that broke it.

**The MFT scanner has now been checked against the walk — and it was the walk that was wrong.** `Mft_scanner_agrees_with_the_walk_when_elevated` needs `$MFT`, and for a long time it never ran: unelevated it returns at its first line, and once the suite declared itself unelevated (above) it returned there elevated too, because it asked the declaration rather than the token. It now asks Windows directly and claims elevation for its own duration only. Run at last, it failed on four files: file counts, directory counts and logical bytes agreed, allocated bytes did not, the MFT saying 10,054 more. That is precisely the cluster rounding of the three ordinary files (3,104 + 3,232 + 3,728) less the 10 bytes of the one resident file — the walk was reporting logical sizes as allocated ones. `GetCompressedFileSize`, which it asked for every file, answers with the logical size for any file that is neither compressed nor sparse; the audit's `DirectoryMeasure` had known that since P2 (§6.4), the scanner had not. Over the 1.2M files of an unelevated walk of this machine's C: that is roughly 2.4 GB understated, booked by §3.5 to *unaccounted*. The walk now rounds up to the cluster and asks only for compressed and sparse files, which also removes 1.2M syscalls from a scan (§4.4); `WalkSizeTests` pins both branches against real files, unelevated, on every run, and the elevated comparison passes with the one-cluster slack it always allowed for.
...
**The navigation is tested the same way, and was not until after v0.2.0.** `IShellWindow` is to the shell what `IPainter` is to a view: a recording implementation, so which view a click selects and what the status line ends up saying can be asserted with no handle and no message loop. It was added for a defect, not for symmetry — see §24.6. The tests there assert what the shell did afterwards, but the assertion that matters is that they return at all: run against the old code they do not fail, they take the test host down with them, which is exactly what they would have done to the release had they existed.
...
**Reclaim** is the report of §7.3 in the two levels of the terminal's screen 4 (§14.7): the rules that matched, largest first, and behind each one its paths. A row carries what deleting it gives back, a bar and a share of what is listed, the rule, **both** axes as one badge — `safe redownload`, `caution irreversible`, because the two together are the point (§7.1) — the table's note column, and how many places matched. A double-click or Enter lists the paths; the word *Reclaim* above the list is the way back, in plain sight like the tree's breadcrumb, and the cursor returns to the rule it left. The risk ceiling is three segments rather than a key that cycles, because a window can show every choice at once and a key hides two of them; `t` still cycles it and `y` still copies the paths, on the terminal screen's letters.

Four decisions in it are about honesty rather than layout. **The headline counts only what pathmemo can delete itself**: a disk image that needs a person and a repository that needs `git gc` are listed with their sizes, dimmed, and named beside the total — *28.5 GB reclaimable · 8.45 GB for another tool or a decision* on this machine — because a total that includes bytes no command of this tool will ever free is the number that never arrives (§7.3). **It reports and does not delete.** What it lists is the unverified index the terminal screens draw their badges from (§7.5) — the rules over the snapshot, without a handle opened per match to ask the guard — and the footer says where deleting, and the guard's check that precedes it, actually happen. A delete button belongs with a delete dialog that shows the plan, the mode and the refusals (§14.1), and that dialog is not in the window yet. **It shares the tree's snapshot** rather than loading its own: one 128 MB snapshot in memory instead of two (§24.7), and one scan behind the tree and the recommendations instead of two. **And it names that scan, and says when it was cut short.** The headline ends in the scan id, because these are stored numbers (§14.7) and because the status line describes the last *completed* scan while the views open the newest snapshot: after a scan that was stopped those are two different scans, and the live check of this tab had both on screen at once. A partial snapshot is *fit for browsing, not for reclaim advice* (§4.8): a cache the scan never reached is simply absent and one it was half way through is listed at half its size, and both look exactly like a complete scan's rows. So above them the tab says the scan was stopped early and that every number is a floor.

**Audit and Duplicates** are not in the window yet, and each tab says which command does that job today. That is the same promise the TUI's screens 4 and 5 made before P7 and P8 filled them in (§14.1) — an unfinished screen that names its replacement is not a dead end.
...
**The reclaim rules run the same way**, for a smaller reason: from the key to the list is between 0.6 and 0.85 s in the published build on a 1.2-million-node snapshot, of which loading the snapshot is about 0.2 (§20) and the thirty-one rules are the rest — forty frames. The tab draws *Applying 31 rules to scan 8…* at once and the list when the worker posts that it is ready — one post, because there is no progress to report, only a result. The index is tagged with the scan it was built from and the shell drops its reference when a scan replaces the snapshot, so a worker that finishes late has nobody to deliver to and cannot put one scan's recommendations over another scan's tree. When a scan finishes while the Tree or the Reclaim tab is showing, that tab reloads: before this, the tree said *run a scan first* one second after a scan had finished.
...
**What that left uncovered, and what it cost.** `IPainter` makes a *view* testable; it does nothing for the *shell* — which view is showing, what a tab click does, what happens when the thing a click asks for cannot be loaded. That was the only part of the window with no tests, because it was the only part that needed a window, and it is the part that shipped a crash in v0.2.0. With nothing scanned yet there is no snapshot to browse, and `Show` handled that by calling `OpenTree`, which handled the failure by calling `Show`: the pair recursed until the stack ran out and Windows killed the process with `0xC00000FD`, two frames short of the message that says to run a scan. Clicking **Tree** did it; so did clicking a volume card. It is a first-run bug — the one state every new user is in — and no amount of testing the treemap was going to find it.

So `IShellWindow` now exists, with a recording implementation beside `RecordingPainter`, and the two methods are split into one that decides and one that only acts: a decision that cannot re-enter itself cannot loop. Both reproduction and fix were checked outside the test suite too, by driving the real window on a store with no snapshots in it.

The Reclaim tab is tested through that seam rather than as a view alone, because the part that can go wrong is between the two: the snapshot the tab has to load, the worker's post, and the frame drawn in the meantime. The recording window does not pump its own posts, so each test decides when the rules "arrive" — which makes the frame *before* they do a deterministic thing to assert instead of a race. The first test is the Tree tab's crash asked of this tab: nothing scanned, the tab selected three times over, and the process still there.

Driving the real window for this tab found two things no test had, which is the argument for doing it every time. The scan card's height was a constant one line short of what is drawn into it, so the directory being read ran underneath **Stop** — the same mistake as the overview's card, which was right for one machine's fonts (§24.3); it is now added up from its contents, and a test holds every line on the card above the button's row, with the MFT scanner's bar and without it. And a scan stopped from this tab left it showing a partial snapshot as if it were a whole one, which is where the warning above came from.
