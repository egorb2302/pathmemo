...
**Статус:** проектная спецификация, готова к реализации
...
Узлы упорядочены так, что **дети каждого каталога идут непрерывным диапазоном** (порядок: DFS, каталоги перед файлами, внутри — по размеру убыв.). Это даёт O(1) навигацию и линейное чтение при отрисовке.
...
|---|---|---|
| `pathmemo-win-x64.exe` | **18–28 МБ** | основной |
| `pathmemo-win-arm64.exe` | 18–28 МБ | отдельный бинарник |
...
| Walk-скан C:, 1.2 млн файлов | < 150 с | degraded-путь, зависит от Defender |
| Инкрементальный USN-скан | **< 2 с** | обычный случай — 0.3 с |
| Запись снимка на диск | < 1.5 с | ~20 МБ deflate |
| Открытие существующего снимка | **< 300 мс** | mmap + распаковка нужных секций |
...
| RSS в TUI со загруженным снимком | < 150 МБ | |
